...
**Owns:** Foreground location acquisition and map-provider integration while preserving provider isolation, privacy, battery discipline and offline resilience.

---

## 1. Mission

Make geographic interaction reliable without letting GPS or map-provider behavior become Wheris's source of truth.

This playbook replaces the former Location and Map skills.

Core principle:

> The saved place is the important data. The map is a representation. Current location is a user-requested capability, not a permanent tracking stream.
...
For current provider/platform behavior consult authoritative current documentation when material:
...
1. Identify whether the task is location acquisition, map presentation, or both.
2. Inspect `:core:location`, `:core:map` and their neutral interfaces/models.
...
6. Confirm whether map/network availability is being incorrectly coupled to GPS availability.
7. Confirm whether the task would introduce background behavior; if so, stop and review scope/security because background location is not part of the current MVP.
...
Do not create continuous history merely because location APIs support it.

### 5.2 Provider isolation

Keep Android/Play Services implementation inside the location infrastructure boundary.

Map provider objects to neutral Wheris concepts such as `UserLocation` / `GeoPoint`.

No `FusedLocationProviderClient` or Android `Location` in domain/ViewModel/general feature contracts.
...
Do not collapse GPS failure, map failure and Internet failure into one generic state.
...
Poor accuracy must be represented honestly and preserve user agency according to the approved flow.

### 5.6 Stale fixes

Never label an old/stale fix as fresh current position without qualification.

If a stale fix is internally useful, preserve correct semantics at the UI boundary.

### 5.7 Permission boundary

Runtime permission request belongs to the Android/UI boundary, not the ViewModel/domain.
...
Keep Mapbox implementation inside `:core:map` or the approved provider boundary.
...
Current user position and saved places must remain visually/semantically distinct.
...
Do not use a full-screen fatal error when list/detail/core local data still works.
...
Keep Light/Dark/provider style configuration centralized and replaceable.

Do not spread provider style IDs throughout features.

### 6.8 Attribution/licensing
...
### 6.9 Sensitive data minimization

Do not send place names, notes, photos or other sensitive content to the map provider merely to render markers if neutral local rendering is sufficient.
...
|---|---|---|---|
| Offline | Available | unavailable/cached | local save may still work if position is usable |
| Online | unavailable | available | saved places/map may still be shown; current-location actions degrade |
| Online | available | unavailable | geographic save can continue without map when approved |
| Offline | unavailable | unavailable | saved local list/detail remain accessible |
...
8. Mapbox Navigation/Directions/Search and Google Maps SDK are not introduced unless product/architecture explicitly changes.
9. Provider cost/lock-in changes must be surfaced before committing architecture.
...
- Mapbox/Play Services/Android tooling versions or provider terms change.

Load `SKILLS/07_MONETIZATION_AND_CLOUD.md` only when:

- advanced offline/cloud/map capabilities are commercially gated or network-backed.
...
Use fakes/neutral adapters where possible instead of real provider/network dependence in unit tests.

Compile affected location/map/features and verify actual stop/cancellation/provider configuration.
...
- provider attribution/terms relevant to the implementation are respected;
...
- claim provider terms/pricing/API behavior from stale knowledge when consequential.
